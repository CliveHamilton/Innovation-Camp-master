 ...
 */
package de.hybris.innovation.cockpits.setup;
...
import de.hybris.innovation.cockpits.constants.InnovationCockpitsConstants;
 ...
 */
@SystemSetup(extension = InnovationCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(InnovationCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(InnovationCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						InnovationCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/innovationcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
